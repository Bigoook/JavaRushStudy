...
async def show_mode_screen(update, context, mode_name, buttons=None, text=None):
    """Показати картинку + текст + кнопки для режиму"""
    await send_image(update, context, mode_name)
    msg = text or load_message(mode_name)
    if buttons:
...
async def ask_gpt(chat_gpt, prompt_name, user_text=None, extra=None):
    """Універсальний виклик GPT з промптом"""
    prompt = load_prompt(prompt_name)
    if extra:
        prompt = f"{prompt}\n\n{extra}"
    return await chat_gpt.send_question(prompt_text=prompt, message_text=user_text)